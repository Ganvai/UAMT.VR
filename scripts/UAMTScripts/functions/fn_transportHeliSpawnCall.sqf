...
if (typename _spawnPosMrk == "ARRAY") then {
	if (typeName (_spawnPosMrk select 0) == "STRING") then {
		{
			_spawnPosArr pushback (getMarkerPos _x);
		}forEach _spawnPosMrk;
	}
	else {
		if (typeName (_spawnPosMrk select 0) == "SCALAR") then {
...
if (typename _despawnPosMrk == "ARRAY") then {
	if (typeName (_despawnPosMrk select 0) == "STRING") then {
		...
		if (typeName (_despawnPosMrk select 0) == "SCALAR") then {
			_destPosArr pushback _despawnPosMrk;
		}
		else {
			_destPosArr = _despawnPosMrk;
		};
	};
}
else {
	_destPosArr pushback (getMarkerPos _despawnPosMrk);
};
...
for "_i" from 0 to _amount do {
	...
	[_heliVeh, _heliGrp, _pickupPos, _destPos, _despawnPos, _voiceLines, _pickUpMapMarker, _destMapMarker] spawn UAMT_fnc_transportHeliFly;
};
