...
if (_pickUpMrk == "") exitWith {
	["Something went wrong with the Pick Up Map Marker. Please restart the Interface", "Error"] spawn BIS_fnc_guiMessage;
};
...
if (_dropOffMrk == "") exitWith {
	["Something went wrong with the Drop Off Map Marker. Please restart the Interface", "Error"] spawn BIS_fnc_guiMessage;
};

if (_lbCurSel == -1) exitWith {
	["No Helicopter selected", "Error"] spawn BIS_fnc_guiMessage;
};

_pickUp = getMarkerPos _pickUpMrk;
if (_pickUp isEqualTo [0,0,0]) exitWith {
	["No Pickup Position set", "Error"] spawn BIS_fnc_guiMessage;
};

_dropOff = getMarkerPos _dropOffMrk;
if (_dropOff isEqualTo [0,0,0]) exitWith {
	["No Drop-Off Position set", "Error"] spawn BIS_fnc_guiMessage;
};
...
if ((((missionNameSpace getVariable "chtHeliArray") select _heliIndex) select 1) == false) exitWith {
	["The Helicopter you wanted to call is currently in use. Coordinate with your group and call another one.", "Error"] spawn BIS_fnc_guiMessage;
};

_result = false;
private _result = ["You are calling a Helicopter Transport? Do you acknowledge?", "Confirm Helicopter Transport Call?", true, true] spawn BIS_fnc_guiMessage;
