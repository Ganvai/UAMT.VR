...
player setVariable ["loadout","AT-Light"];
_notloadeditems = [];
...
//	Outfit
//------------------------------------------------------------------
player forceAddUniform launcher_uniform;
player addHeadgear launcher_helmet;
player addVest launcher_vest;
...
if (lrAvailable && ((player getVariable "loadout") in lrRoles) && (lrRadio != "")) then {
	...
	if (isClass(configFile >> "cfgPatches" >> "acre_main")) then {
		...
		player addBackpack launcher_backpack;
...
player addHandgunItem launcher_at_light_attachement1;
player addHandgunItem launcher_at_light_attachement2;
player addHandgunItem launcher_at_light_attachement3;
player addHandgunItem ((launcher_at_light_ammo select 0) select 0);
...
player linkItem launcher_binocs;
